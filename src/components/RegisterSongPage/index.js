...
const RegisterSongPage = () => {
  ...
  const albumImageValidation = () => {
    if (
      !albumImg.includes("jpg") ||
      !albumImg.includes("png") ||
      !albumImg.includes("jpeg")
    ) {
      errorMessageAnimation(
        "Formato de imagem inválido! Insira uma foto JPG, JPEG ou PNG."
      );
      return false;
    }
  ...
  const urlOrFileVerification = () => {
    if (url && !file) {
      if (url.includes("https://") && url.includes("youtu")) {
        ...
        errorMessageAnimation(
          "URL inválida! Por favor, insira uma URL do YouTube e tente novamente."
        );
        return false;
      }
    }
    if (file && !url) {
      if (file.name.includes("mp3")) {
        ...
        errorMessageAnimation(
          "Formato de arquivo inválido! Por favor, insira um arquivo MP3 e tente novamente."
        );
        return false;
      }
    }
  ...
  const handleUploadFile = (data) => {
    axios
      .post(
        `http://ec2-34-204-93-195.compute-1.amazonaws.com:7999/upload/${fileName}`,
        data,
        {}
      )
      .then((res) => {
    ...
    if (
      ...
      try {
        ...
        if (
          ...
          "This song is already registered in the database!"
        ) {
          errorMessageAnimation(
            "Esta música já está registrada no banco de dados!"
          );
        }
        console.log(err.response.data.error);
      }
