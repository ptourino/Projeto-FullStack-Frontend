...
const HomePage = () => {
  ...
  const [headerMessage, setHeaderMessage] = useState(
    "Todas as músicas em ordem ascendente"
  );
  ...
  const getColor = () => {
    getAverageColor("https://cors-anywhere.herokuapp.com/" + albumImage).then(
      (rgb) => {
        setPalette(rgb);
  ...
  const deleteSongById = async (song) => {
    if (
      window.confirm(
        `Você tem certeza que deseja deletar a música: '${song.title}'?`
      )
    ) {
      try {
        await axios.delete(`${baseUrl}/music/delete/${song.id}`, axiosConfig);
        successMessageAnimation(
          `A música: '${song.title}' foi deletada com sucesso!`
        );
        getSongs();
      } catch (err) {
  ...
  const setURLQuery = (
    ...
    if (!userSongs && genre.length > 1) {
      setHeaderMessage(
        `Todas as músicas por gênero '${genre}' em ordem ascendente`
      );
    }
    if (userSongs && genre.length > 1) {
      setHeaderMessage(
        `Suas músicas por gênero '${genre}' em ordem ascendente`
      );
    }

    if (userSongs && genre.length > 1 && title && order === "ASC") {
      setHeaderMessage(
        `Suas músicas por título '${title}', gênero '${genre}' em ordem ascendente`
      );
    }
