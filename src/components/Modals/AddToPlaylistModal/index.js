...
const SongModal = (props) => {
  ...
  const addSongToPlaylist = async (playlist) => {
    ...
    try {
      await axios.post(`${baseUrl}/playlist/add/${song.id}`, body, axiosConfig);
      setSuccess(true);
      setMessage(
        `Música '${song.title}' adicionada com sucesso à playlist '${playlist.title}'`
      );
    } catch (err) {
      setSuccess(false);
      setError(
        `Erro ao adicionar a música '${song.title}' à playlist '${playlist.title}' `
      );
      console.log(err);
    }
  };
